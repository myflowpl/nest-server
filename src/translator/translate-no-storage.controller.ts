...
export class TranslateController {
  ...
  async translateWord(@Query() data: TranslateDto, @Auth() user: User): Promise<TranslateResponse> {
    ...
    console.log('user', user.name, 'translates', data.word, 'to', data.translateFrom);
    
    const dir = (data.translateFrom === Languages.EN) ? this.en : this.pl;

    const translation = dir[data.word.toLowerCase()];

    return {
      translation,
      language: data.translateFrom
    }
  }
